...
def compute_sdm(image_fetures, text_fetures, pid, logit_scale, image_id=None, factor=0.3, epsilon=1e-8):
    ...
    """
    image_fetures = torch.cat(GatherLayer.apply(image_fetures), dim=0)
    text_fetures = torch.cat(GatherLayer.apply(text_fetures), dim=0)
    pid = torch.cat(GatherLayer.apply(pid), dim=0)
...
# def compute_id(image_logits, labels):
#
#     criterion = CrossEntropyWithLabelSmooth()
#
